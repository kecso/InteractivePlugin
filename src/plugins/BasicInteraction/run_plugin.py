"""
This script is called by the plugin-wrapper, BasicInteraction.js, which passes down the
...
Notes:
 ...
 - For the two-way messaging example, the communication file argument is added.
"""
...
COMMUNICATION_FILE = sys.argv[7].strip('"')
...
logger.debug('communication-file: {0}'.format(COMMUNICATION_FILE))

# Create an instance of WebGME and the plugin
webgme = WebGME(PORT, logger)
plugin = BasicInteraction(webgme, COMMIT_HASH, BRANCH_NAME, ACTIVE_NODE_PATH, ACTIVE_SELECTION_PATHS, NAMESPACE)

# initiate the plugin communication channels
plugin.setup_communication(COMMUNICATION_FILE)
# Do the work
plugin.main()
